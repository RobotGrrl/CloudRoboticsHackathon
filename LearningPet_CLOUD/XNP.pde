...
int XNPparseIncomingPacket() {
  ...
  if(payload[0] == 'H') {
    wearinghat = ((int)payload[1])-48;
    recu = true;
  }
  
  /*
  if(destination[0] == 'A' && destination[1] == 'L' && destination[2] == 'L') {
    Serial.println("pulse rx");
    pulse = payload[0];
    recu = true;
  }
  
  if(destination[0] == 'L' && destination[1] == 'P' && payload[0] == 'H') {
    Serial.println("hat rx");
    hat = ((int)payload[1])-48;
  }
  */
